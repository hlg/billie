...
public class MefistoDemo {
    ...
    private static pm = new SimplePluginManager();
    static {
    ...
    public void loadGantt(JScrollPane panel) {
        ...
        LinkedList<String> modelIds = dataAcessor.readFromFolder(new File(fileName.text), new EMTypeCondition(EMTypes.ACTIVITY11),
    ...
    private void loadHEB(JScrollPane panel){
        ...
        LinkedList<String> ids = dataAcessor.readFromFolder(new File(fileName.text), "L2", new EMTypeCondition(EMTypes.QTO) {
            @Override
            public boolean isValidFor(ElementaryModel model) {
    ...
    private void loadBarchart(JScrollPane panel){
        ...
        List<String> modelIds = data.readFromFolder(new File(fileName.text), new EMTypeCondition(EMTypes.GAEB));
    ...
    private void load3dModel(JScrollPane panel) {
        ...
        accessor.readFromFolder(new File(fileName.text))
            ...
            frame(title: 'Mefisto Vis Demo', show: true, defaultCloseOperation: JFrame.EXIT_ON_CLOSE, extendedState: JFrame.MAXIMIZED_BOTH, iconImages: icons ) {
                panel(id: 'mainPanel', constraints: BL.NORTH) {
                    ...
                    button(text: "Select", action {
                        ...
                        fileName.text = chooser.selectedFile
                    })
                    button(text: 'Load', action {
                        println "Loading $fileName.text"
                        [p0,p1,p2,p3].each{p ->
                            p.getViewport().add( panel(layout: flowLayout(alignment: FL.LEFT)){
                            progressBar(background: Color.WHITE, indeterminate: true, progressString: 'loading', stringPainted: true)} ) // label(text: 'loading ...')
                        }
                        doOutside {
                            loadBarchart(p0)
                            loadGantt(p1)
